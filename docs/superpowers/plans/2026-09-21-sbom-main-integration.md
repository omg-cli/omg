...
Hosted Linux tests at be0ba328 completed, but admission rejected eight reviewed CLI fault contracts because only daemon fault kinds were allowed. An explicit seven-test CLI fault allowlist now admits those eight contracts; an unrelated runtime test still cannot claim fault evidence. The new regression failed before the correction. Diagnostic replay of Ubuntu's saved hosted selection produces45 passing CLI receipts, not fresh hosted proof for the amended adapter.

QEMU Debian's SBOM row instead failed because its offline-refusal assertion retained an online network policy. The guest successfully generated338 components, correctly causing the refusal oracle to fail. Rename the scenario `audit-sbom-offline` and register a new inventory policy with offline scope; preserve the previous hash and policy unchanged for historical evidence. The network-policy regression failed before correction. Combined Windows admission/policy verification executes27 passing tests with one Linux-runuser skip. Fresh hosted gates remain mandatory.
