...
class ReleaseRecipeAlignment(unittest.TestCase):
    def test_ci_release_cpu_baseline_matches_arch_and_generic_distros(self):
        ...
        for distro, cpu in (('arch', '-C target-cpu=x86-64-v2'), ('debian', ''), ('fedora', '')):
            script = command.replace('${{ matrix.features }}', distro + ',pgp,license')
            ...
            with self.subTest(distro=distro):
                self.assertEqual(result.stdout.strip(), cpu)
