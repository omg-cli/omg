...
mod cache {
	...
	struct AptTestConfig(std::path::PathBuf);

	impl Drop for AptTestConfig {
		fn drop(&mut self) {
			Config::new().reset();
	...
	fn use_package_fixture_repository() -> AptTestConfig {
		let temp_dir = std::env::temp_dir().join(format!(
			"rust-apt-package-fixtures-{}-{}",
			std::process::id(),
		...
		std::fs::create_dir_all(lists_dir.join("partial")).unwrap();

		let repository = common::cache_dir().canonicalize().unwrap();
		let sources = temp_dir.join("sources.list");
		std::fs::write(
			&sources,
			format!("deb [trusted=yes] file:{} ./\n", repository.display()),
		)
		.unwrap();

		let config = Config::new_clear();
		config.set("Dir::Etc::sourcelist", sources.to_str().unwrap());
		config.set("Dir::Etc::sourceparts", "-");
		config.set("Dir::State::lists", lists_dir.to_str().unwrap());
		config.set("Dir::Cache::pkgcache", "");
		config.set("Dir::Cache::srcpkgcache", "");
		config.set("Acquire::Languages", "none");

		let cache = new_cache!().unwrap();
		cache.update(&mut AcquireProgress::quiet()).unwrap();

		AptTestConfig(temp_dir)
	}
	...
	fn with_debs() {
		let cache = new_cache!(&[
			common::cache_file("apt.deb"),
			common::cache_file("dep-pkg1_0.0.1.deb"),
		])
		.unwrap();
		...
		assert!(new_cache!(&[common::cache_file("this-file-doesnt-exist.deb")]).is_err());

		// Check if it errors on a garbage empty file as well
		// signal: 11, SIGSEGV: invalid memory reference
		assert!(new_cache!(&[common::cache_file("pkg.deb")]).is_err());
	}

	#[test]
	fn with_packages() {
		let cache = new_cache!(&[common::cache_file("Packages")]).unwrap();
		...
		assert!(new_cache!(&[common::cache_file("Packages.gz")]).is_err());
	}

	#[test]
	fn update_falls_back_to_release_and_detached_signature() {
		let temp_dir = std::env::temp_dir().join(format!(
			"rust-apt-release-fallback-{}-{}",
			std::process::id(),
			std::time::SystemTime::now()
				.duration_since(std::time::UNIX_EPOCH)
				.unwrap()
		...
		let _cleanup = AptTestConfig(temp_dir.clone());
		let fixture_dir = std::path::Path::new("tests/files/release-fallback");
		let repo = fixture_dir.join("repo").canonicalize().unwrap();
		let lists_dir = temp_dir.join("lists");
		let config = Config::new_clear();
		assert!(!repo.join("dists/test/InRelease").exists());
		...
		std::fs::write(
			&sources,
			format!(
				"deb [arch=amd64 signed-by={}] file:{} test main\n",
				fixture_dir
					.join("test-key.asc")
					.canonicalize()
					.unwrap()
		...
		let cache = new_cache!().unwrap();
		assert!(cache.get("rust-apt-release-fallback-canary").is_some());
	}
	...
	#[test]
	fn no_description() {
		let cache = new_cache!(&[common::cache_file("no-description_0.0.1.deb")]).unwrap();
	...
	#[test]
	fn version_uris() {
		let _config = use_package_fixture_repository();
		let cache = new_cache!().unwrap();
		let pkg = cache.get("apt").unwrap();
	...
	fn hashes() {
		...
		assert!(version.hash("sha1").is_some())
	}
	...
	fn depcache_install_ver() {
		let cache = new_cache!(&[
			common::cache_file("dep-pkg1_0.0.1.deb"),
			common::cache_file("dep-pkg1_0.0.2.deb"),
		])
		.unwrap();
	...
	#[test]
	fn broken_pkgs() {
		let cache = new_cache!(&[common::cache_file("broken-or-dep_0.0.1.deb")]).unwrap();
