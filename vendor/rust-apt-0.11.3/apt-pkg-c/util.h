...
inline i32 cmp_versions(str ver1, str ver2) {
	...
	// APT's numeric fragment loops can inspect the byte at the end pointer.
	// Rust slices need not end at a NUL, so provide owned terminated copies.
	const std::string left(ver1.data(), ver1.size());
	const std::string right(ver2.data(), ver2.size());
	return _system->VS->DoCmpVersion(left.data(), left.data() + left.size(),
